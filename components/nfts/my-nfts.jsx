...
const MyNfts = () => {
  ...
  const formatNftMetadata = (nftIds, tokenURIs) => {
    return nftIds.map((nftId, i) => {
      return {
        tokenId: nftId,
  ...
  const getOwnedNfts = async () => {
    if (!isRefreshing) {
      try {
        ...
        const tokenURIPromises = nftIds.map(async nftId => {
    ...
    <div>
      ...
      {nftData && nftData.length > 0 ? (
        <div className="nft-list">
          {nftData.map(nft => {
            return (
              <NFT id={nft.tokenId} key={nft.tokenId} name={nft.tokenURI} />
            )
          })}
        </div>
      ) : (
